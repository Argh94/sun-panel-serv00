...
# Version: 1.1.0
# Date: June 7, 2025
# Updated to include dependency checking and installation
...
checkAndInstallDependencies() {
  ...
  local tools=("curl" "grep" "awk" "nohup")
  
  # Check if running on FreeBSD
  if [ "$(uname -s)" != "FreeBSD" ]; then
    red "This script is designed for FreeBSD systems only!"
    exit 1
  fi
  
  # Check for pkg (FreeBSD package manager)
  if ! command -v pkg >/dev/null 2>&1; then
    red "Package manager 'pkg' not found! Please install it manually."
    exit 1
  fi
  
  # Update pkg repository
  green "Updating package repository..."
  if ! pkg update >/dev/null 2>&1; then
    red "Failed to update package repository! Check your internet connection or permissions."
    exit 1
  fi
  ...
  for tool in "${tools[@]}"; do
    if ! command -v "$tool" >/dev/null 2>&1; then
      ...
      if ! pkg install -y "$tool" >/dev/null 2>&1; then
        red "Failed to install $tool! Check your internet connection or permissions."
        exit 1
      fi
      green "$tool installed successfully!"
    else
      green "$tool is already installed."
    fi
  done
  
  # Note: ps is part of FreeBSD base system, so no need to install it
  green "All dependencies are installed and up-to-date!"
}
...
# Check and install dependencies before showing the menu
checkAndInstallDependencies
